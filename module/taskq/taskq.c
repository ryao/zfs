...
/*
 * XXX: The instance ids are not meaningful for our purposes on Linux, but
 * until we do some refactoring to make this more cross platform friendly, we
 * just #ifdef it out, assign everything an id of 0 and add this comment.
 */
...
/*
 * Some compatibility code for Linux
 */
#ifdef __linux__
#include <linux/sched.h>
#define	SSYS	PF_KTHREAD
#define	p_flag	flags
#define t_taskq journal_info
#define p_pid	pid

/*
 * XXX: CPU is intended to be a pointer to the current `struct cpu` on Solaris,
 * but the Linux equivalent is not available to us
 */
...
taskq_bucket_extend(void *arg)
{
	...
	tqe->tqent_thread->t_taskq = tq;
	wake_up_process(tqe->tqent_thread);
#else
# error "Implement code to wake-up kernel thread."
#endif /* __sun__ */
}
...
/*
 * The taskq code is an integral part of the Solaris kernel, such that it
 * should always be available. However, it is a foreign module on other kernels
 * and as such, should be unloadable.
 */
...
#if defined(__linux__) && defined(_KERNEL) && defined(HAVE_SPL)
...
spl_module_init(ztask_init);
spl_module_exit(ztask_fini);

MODULE_DESCRIPTION("Generic task queue implementation");
...
EXPORT_SYMBOL(taskq_create_instance);
EXPORT_SYMBOL(taskq_create_proc);
EXPORT_SYMBOL(taskq_create_sysdc);
EXPORT_SYMBOL(nulltask);
EXPORT_SYMBOL(taskq_dispatch);
EXPORT_SYMBOL(taskq_dispatch_ent);
EXPORT_SYMBOL(taskq_destroy);
EXPORT_SYMBOL(taskq_wait);
EXPORT_SYMBOL(taskq_suspend);
EXPORT_SYMBOL(taskq_suspended);
EXPORT_SYMBOL(taskq_resume);
