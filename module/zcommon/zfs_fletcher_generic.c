...
ZFS_NO_SANITIZE_UNDEFINED
static void
fletcher_4_generic_native_impl(fletcher_4_ctx_t *ctx, const void *buf,
    uint64_t size)
{
	const u32x4 *ip = buf;
...
ZFS_NO_SANITIZE_UNDEFINED
static void
fletcher_4_generic_byteswap_impl(fletcher_4_ctx_t *ctx, const void *buf,
    uint64_t size)
{
	const u32x4 *ip = buf;
	const u32x4 *ipend = (u32x4 *)((uint8_t *)ip + size);
...
{
	kfpu_begin();
	fletcher_4_generic_native_impl(ctx, buf, size);
	kfpu_end();
}
...
{
	kfpu_begin();
	fletcher_4_generic_byteswap_impl(ctx, buf, size);
	kfpu_end();
}
